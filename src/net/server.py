"""
socket manager by 'cfreshman' https://github.com/cfreshman/pico-fi/blob/m/src/lib/server.py
"""
...
from net import defaulter_dict, enumstr
...
class Transport:
    ...
    def of(sock_type: int):
        return {socket.SOCK_DGRAM: Transport.UDP, socket.SOCK_STREAM: Transport.TCP}[
            sock_type
        ]
...
class connection:
    ...
    def __repr__(self):
        return f"<connection {self.tran} {id(self.sock)}>"
    ...
    def __init__(self, value, transport: transport):
        ...
        if self in self._transports:
            if self._transports[self] != transport:
                raise Exception(
                    f"multiple transports ({self._transports[self], transport}) for protocol {self}"
                )
        else:
...
class Orchestrator(SocketPollHandler):
    ...
    def unregister(
        ...
        if self.handlers[conn] == handler:
            del self.handlers[conn]
    ...
    def handle(self, sock: socket.socket, event):
        try:
            ...
            if isinstance(handler, transport):
                handler = self.handlers.get(handler)
            if isinstance(handler, protocol):
                handler = self.handlers.get(handler)
            if handler and not isinstance(handler, SocketPollHandler):
                raise Exception(
                    f"failed handler resolution (sock -> transport -> protocol -> handler), ended with protocol {handler}"
                )

            if handler:
                return handler.handle(sock, event)
            return True
        except OSError as e:
...
class Server(ProtocolHandler):
    def __init__(
        self, orch: Orchestrator, port: int, proto: protocol, name: str = None
    ):
        ...
        )  # allow overlapped reads

        self.orch.register(self.conn, self)  # register as handler for connection
        self.poller.register(
            self.sock, select.POLLIN
        )  # register socket for event polling
...
class IpSink:
    def __init__(self, ip: bytes or str):
        self.ip = encode(ip) if ip else None
